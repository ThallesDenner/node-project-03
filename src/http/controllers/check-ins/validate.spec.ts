...
describe('Validate Check-in (e2e)', () => {
  ...
  it('should be able to validate a check-in', async () => {
    ...
    // Criação de check-in (isto não é o ideal, pois estamos criando o check-in sem passar pelas camadas de controle e serviço)
    ...
    // Solicitação HTTP para a aplicação (validar check-in)
    const response = await request(app.server)
      .patch(`/check-ins/${checkIn.id}/validate`)
      .set('Authorization', `Bearer ${token}`)
      .send()

    // Verifica se a resposta da API possui um código de status HTTP 204 (Success), indicando que a requisição foi bem sucedida e o cliente não precisa sair da página atual.
    expect(response.statusCode).toEqual(204)

    // Busca de check-in pelo id (isto não é o ideal, pois estamos buscando o check-in sem passar pelas camadas de controle e serviço)
